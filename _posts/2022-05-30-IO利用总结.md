...
### House OF Emma(largebinattack/houseofkiwi/2.34uaf)
...
关于_IO_cookie_file这个结构体是 _IO_FILE_plus 的扩展，如果我们可以控制 IO 的内容，大概率这部分的数据也是可控的，并且其的第一个参数也是来源于这个结构。所以我们可以把其当做一个**类似于 __free_hook 的 Hook 来利用**。
...
这里调用的函数指针是有指针保护的，一个类异或加密，所以我们要反加密一下，其值存在fs:[0x30]处
...
这种时候就可以考虑构造两个 IO_FILE，且后者指针处于前者的 _chains 处，前者用 [GLIBC2.34 之前的 IO_FILE 攻击](https://www.anquanke.com/post/id/216290)的思想在 __pointer_chk_guard 处写已知内容，后者再用 House OF Emma 来进行函数指针调用。

> 这部分内容大多来自wjh师傅的博客，这里只是稍加注解<https://blog.wjhwjhn.com/archives/751/>
