...
方案一：劫持IO_list_all，如果是fastbin reverse就劫持IO_list_all+0x70(stderr的chain字段)
...
方案二：构造堆复用，实现largebinattack->劫持IO_list_all
...
+ 2.34以后版本：没有了free_hook和malloc_hook，就不考虑劫持hook，根据是否可以执行刷新流，可以就者用house of pig 以及house of emma，不可以就用house of kiwi
